...
class ExecutionConfig:
    ...
    numba_threads: int = 0
    phase3_production_lite: bool = False
    nested_diag_stride: int = 500
    workers: int = 1
    local_active_window: bool = True
    local_active_threshold_m: float = 0.02
    local_active_prebuffer_sec: float = 600.0
    local_active_postbuffer_sec: float = 900.0
...
class PreparedDomain:
    ...
    edge_wet_masks: Optional[Dict[str, np.ndarray]] = None
    edge_depth_m: Optional[Dict[str, np.ndarray]] = None
    local_connectivity: Optional[Dict[str, Any]] = None
...
class LocalNestedOutput:
    ...
    dapa_p95_inun: float
    meta: Dict[str, Any]
    gauge_max_abs: Dict[str, float] = field(default_factory=dict)
...
def compute_edge_wet_info(H: np.ndarray, min_depth: float) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray], Dict[str, int]]:
    masks = {
        "bottom": np.asarray(H[0, :] > min_depth, dtype=bool),
        "top": np.asarray(H[-1, :] > min_depth, dtype=bool),
        "left": np.asarray(H[:, 0] > min_depth, dtype=bool),
        "right": np.asarray(H[:, -1] > min_depth, dtype=bool),
    }
    depths = {
        "bottom": np.asarray(H[0, :], dtype=np.float64),
        "top": np.asarray(H[-1, :], dtype=np.float64),
        "left": np.asarray(H[:, 0], dtype=np.float64),
        "right": np.asarray(H[:, -1], dtype=np.float64),
    }
    counts = {k: int(np.count_nonzero(v)) for k, v in masks.items()}
    return masks, depths, counts


def analyze_local_connectivity(grid: Grid2D, H: np.ndarray, min_depth: float, gauge: GaugePoint) -> Dict[str, Any]:
    wet = np.asarray(H > min_depth, dtype=np.uint8)
    ny, nx = wet.shape
    ...
    while head < len(queue):
        ...
        for di, dj in nbrs:
            ...
            if 0 <= ii < ny and 0 <= jj < nx and wet[ii, jj] and not visited[ii, jj]:
                visited[ii, jj] = 1
                queue.append((ii, jj))
    gi = nearest_index(np.asarray(grid.lat), gauge.lat)
    gj = nearest_index(np.asarray(grid.lon), gauge.lon)
    connected_target = bool(visited[gi, gj]) if 0 <= gi < ny and 0 <= gj < nx else False
    return {
        "wet_cells": int(np.count_nonzero(wet)),
        "boundary_connected_wet_cells": int(np.count_nonzero(visited)),
        "target_i": int(gi),
        "target_j": int(gj),
        "target_is_wet": bool(wet[gi, gj]) if 0 <= gi < ny and 0 <= gj < nx else False,
        "target_connected_to_boundary": connected_target,
    }


def compute_driver_diagnostics(driver: Dict[str, np.ndarray], local_domain: PreparedDomain) -> Dict[str, Any]:
    edge_counts = {k: int(np.count_nonzero(local_domain.edge_wet_masks.get(k, np.zeros(1, dtype=bool)))) for k in ("bottom", "top", "left", "right")} if local_domain.edge_wet_masks else {}
    peaks = {f"peak_{k}_eta_m": float(np.max(np.abs(driver[k]))) if driver[k].size else 0.0 for k in ("bottom", "top", "left", "right")}
    all_peak = max(peaks.values()) if peaks else 0.0
    return {**edge_counts, **peaks, "peak_boundary_eta_m": float(all_peak)}


def impose_boundary_state_local(q: np.ndarray, eta_bc: Dict[str, np.ndarray], local_domain: PreparedDomain, gravity: float) -> None:
    masks = local_domain.edge_wet_masks or {}
    depths = local_domain.edge_depth_m or {}
    q[1, 0, :] = 0.0
    q[1, -1, :] = 0.0
    q[1, :, 0] = 0.0
    q[1, :, -1] = 0.0
    ...
    mask = masks.get("bottom")
    if mask is not None and np.any(mask):
        q[0, 0, mask] = eta_bc["bottom"][mask]
        h = np.maximum(depths["bottom"][mask], local_domain.dx * 0.0 + 0.10)
        q[2, 0, mask] = eta_bc["bottom"][mask] * np.sqrt(gravity * h)
    mask = masks.get("top")
    if mask is not None and np.any(mask):
        q[0, -1, mask] = eta_bc["top"][mask]
        h = np.maximum(depths["top"][mask], local_domain.dx * 0.0 + 0.10)
        q[2, -1, mask] = -eta_bc["top"][mask] * np.sqrt(gravity * h)
    mask = masks.get("left")
    if mask is not None and np.any(mask):
        q[0, mask, 0] = eta_bc["left"][mask]
        h = np.maximum(depths["left"][mask], local_domain.dx * 0.0 + 0.10)
        q[1, mask, 0] = eta_bc["left"][mask] * np.sqrt(gravity * h)
    mask = masks.get("right")
    if mask is not None and np.any(mask):
        q[0, mask, -1] = eta_bc["right"][mask]
        h = np.maximum(depths["right"][mask], local_domain.dx * 0.0 + 0.10)
        q[1, mask, -1] = -eta_bc["right"][mask] * np.sqrt(gravity * h)
...
def prepare_domain(cfg: ProjectConfig) -> PreparedDomain:
    ...
    if cfg.execution.local_dapa_nested and local_merged is not None:
        ...
        edge_wet_masks, edge_depth_m, edge_counts = compute_edge_wet_info(local_H, max(cfg.solver.min_depth_m, 0.10))
        connectivity = analyze_local_connectivity(local_merged, local_H, max(cfg.solver.min_depth_m, 0.10), GaugePoint("Dapa_virtual", DAPA_MARKER[0], DAPA_MARKER[1]))
        ...
        print(
            ...
            f"wet_cells={int(np.count_nonzero(local_H > cfg.solver.min_depth_m))} edge_wet(bottom/top/left/right)="
            f"{edge_counts['bottom']}/{edge_counts['top']}/{edge_counts['left']}/{edge_counts['right']} "
            f"target_connected_to_boundary={connectivity['target_connected_to_boundary']}",
            flush=True,
        )
...
def apply_phase3_production_lite_preset(cfg: ProjectConfig) -> None:
    cfg.execution.phase3_production_lite = True
    cfg.execution.save_per_sample_maps = False
    cfg.execution.write_png = False
    cfg.execution.write_geotiff = False
    cfg.execution.write_npz = True
    cfg.execution.emit_phase4_preview = False
    cfg.execution.checkpoint_every = max(4, min(cfg.execution.checkpoint_every, 8))
    cfg.execution.local_active_window = True
    cfg.solver.t_end_sec = 7200.0
    cfg.execution.workers = max(4, int(cfg.execution.workers))
...
def trim_driver_to_active_window(driver: Dict[str, np.ndarray], cfg: ProjectConfig) -> Tuple[Dict[str, np.ndarray], Dict[str, float]]:
    if (not cfg.execution.local_active_window) or driver["time"].size <= 2:
        return driver, {"trimmed": False, "t_start_sec": float(driver["time"][0]), "t_end_sec": float(driver["time"][-1])}
    env = np.maximum.reduce([
        np.max(np.abs(driver["top"]), axis=1),
        np.max(np.abs(driver["bottom"]), axis=1),
        np.max(np.abs(driver["left"]), axis=1),
        np.max(np.abs(driver["right"]), axis=1),
    ])
    idx = np.flatnonzero(env >= cfg.execution.local_active_threshold_m)
    if idx.size == 0:
        return driver, {"trimmed": False, "t_start_sec": float(driver["time"][0]), "t_end_sec": float(driver["time"][-1]), "peak_boundary_eta_m": float(np.max(env)) if env.size else 0.0}
    tt = driver["time"]
    t0 = max(float(tt[0]), float(tt[idx[0]]) - cfg.execution.local_active_prebuffer_sec)
    t1 = min(float(tt[-1]), float(tt[idx[-1]]) + cfg.execution.local_active_postbuffer_sec)
    i0 = max(0, int(np.searchsorted(tt, t0, side="left")))
    i1 = min(tt.size - 1, int(np.searchsorted(tt, t1, side="right") - 1))
    out = {k: v[i0:i1+1].copy() for k, v in driver.items()}
    return out, {
        "trimmed": True,
        "t_start_sec": float(out["time"][0]),
        "t_end_sec": float(out["time"][-1]),
        "original_t_start_sec": float(tt[0]),
        "original_t_end_sec": float(tt[-1]),
        "peak_boundary_eta_m": float(np.max(env)) if env.size else 0.0,
    }
...
def run_local_nested_simulation(cfg: ProjectConfig, local_domain: PreparedDomain,
    ...
    t = 0.0
    step = 0
    total_steps = max(1, int(math.ceil(t_end / max(dt0, 1.0e-12))))
    ...
    local_gauge_max_abs = {name: 0.0 for name in local_domain.gauge_interp}
    while t < t_end:
        dt = min(dt0, t_end - t)
        eta_bc = interpolate_boundary_state(driver, t)
        impose_boundary_state_local(q, eta_bc, local_domain, cfg.solver.gravity)
        ...
        if use_numba:
            ...
            impose_boundary_state_local(q, eta_bc_next, local_domain, cfg.solver.gravity)
            ...
            q1 = q + dt * k1
            eta_bc_next = interpolate_boundary_state(driver, t + dt)
            impose_boundary_state_local(q1, eta_bc_next, local_domain, cfg.solver.gravity)
            ...
            arrival_time[newly_wet] = np.float32(t)
            wet_mask |= wet.astype(np.uint8)
        for gname, spec in local_domain.gauge_interp.items():
            try:
                val = abs(bilinear_sample(eta, spec))
            except Exception:
                val = 0.0
            if val > local_gauge_max_abs[gname]:
                local_gauge_max_abs[gname] = float(val)
        step += 1
        if pbar is not None and step % max(1, cfg.execution.nested_diag_stride) == 0:
            maybe_print(f"    [LOCAL NEST] step {step}/{total_steps}  t={t + dt:8.1f}s", pbar)
        t += dt
    ...
    return LocalNestedOutput(
        ...
        gauge_max_abs=local_gauge_max_abs,
    )
...
def build_viability_report(cfg: ProjectConfig, domain: PreparedDomain, p95_map: np.ndarray, dapa_p95_df: pd.DataFrame,
                           local_p95_map: Optional[np.ndarray] = None, local_domain: Optional[PreparedDomain] = None,
                           catalog_df: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
    ...
    out = {
        ...
        "regional_dapa_nonzero_inundation_cells": int(np.count_nonzero((p95_map[domain.dapa_ys, domain.dapa_xs] > 0.0) & (domain.grid.z[domain.dapa_ys, domain.dapa_xs] > 0.0))),
        "regional_dapa_p95_positive_depth_m": float(np.percentile(p95_map[domain.dapa_ys, domain.dapa_xs][(p95_map[domain.dapa_ys, domain.dapa_xs] > 0.0) & (domain.grid.z[domain.dapa_ys, domain.dapa_xs] > 0.0)], 95)) if np.count_nonzero((p95_map[domain.dapa_ys, domain.dapa_xs] > 0.0) & (domain.grid.z[domain.dapa_ys, domain.dapa_xs] > 0.0)) else 0.0,
    }
    if local_domain is not None:
        out["local_edge_wet_counts"] = {k: int(np.count_nonzero(v)) for k, v in (local_domain.edge_wet_masks or {}).items()}
        out["local_connectivity"] = local_domain.local_connectivity
    if catalog_df is not None and len(catalog_df):
        gauge_cols = [c for c in catalog_df.columns if c.startswith("gauge_max_abs_")]
        if gauge_cols:
            out["mean_max_wave_by_gauge_m"] = {c.replace("gauge_max_abs_", "").replace("_m", ""): float(catalog_df[c].mean()) for c in gauge_cols}
        if "peak_boundary_eta_m" in catalog_df.columns:
            out["mean_peak_local_boundary_eta_m"] = float(catalog_df["peak_boundary_eta_m"].mean())
            out["min_peak_local_boundary_eta_m"] = float(catalog_df["peak_boundary_eta_m"].min())
        if "local_nonzero_inundation_cells" in catalog_df.columns:
            out["mean_local_nonzero_inundation_cells"] = float(catalog_df["local_nonzero_inundation_cells"].mean())
    return out
...
def scenario_summary_row(mw: float, sample: SampleOutput, runtime_sec: float, domain: PreparedDomain, use_local: bool) -> Dict[str, Any]:
    ...
    gauge_vals = {k: float(v) for k, v in sample.gauge_max_abs.items()}
    if use_local and sample.local_nested is not None and sample.local_nested.gauge_max_abs:
        for k, v in sample.local_nested.gauge_max_abs.items():
            gauge_vals[k] = float(v)
    return {
        "mw": mw,
        ...
        **{f"gauge_max_abs_{k}_m": float(v) for k, v in gauge_vals.items()},
    }
...
_WORKER_CFG: Optional[ProjectConfig] = None
_WORKER_DOMAIN: Optional[PreparedDomain] = None


def persist_phase3_sample_outputs(cfg: ProjectConfig, domain: PreparedDomain, sample: SampleOutput, mw: float, sample_index: int, slip_name: str, mag_dir: Path) -> Dict[str, str]:
    sample_dir = mag_dir / f"sample_{sample_index:04d}_{slip_name}"
    ensure_dir(sample_dir)
    save_gauge_series(sample_dir, sample)
    save_json(sample_dir / "sample_meta.json", sample.meta)
    regional_depth_path = sample_dir / "regional_max_depth.npy"
    np.save(regional_depth_path, sample.max_depth.astype(np.float32))
    outputs = {"regional_depth_npy": str(regional_depth_path)}
    if cfg.execution.phase3_production_lite:
        if sample.local_nested is not None and domain.nested_local_domain is not None:
            ...
            local_depth_path = sample_dir / "local_max_depth.npy"
            np.save(local_depth_path, sample.local_nested.max_depth.astype(np.float32))
            outputs["local_depth_npy"] = str(local_depth_path)
        else:
            ...
            export_map_bundle(sample_dir, f"{slip_name}_dapa_corridor", dapa_grid,
                              ...
                              sample.wet_mask[domain.dapa_ys, domain.dapa_xs],
                              write_geotiff=cfg.execution.write_geotiff, write_npz=cfg.execution.write_npz)
    elif sample_index == 0 or cfg.execution.save_per_sample_maps:
...
def _init_phase3_worker(cfg: ProjectConfig) -> None:
    global _WORKER_CFG, _WORKER_DOMAIN
    _WORKER_CFG = cfg
    if NUMBA_AVAILABLE:
        try:
            set_num_threads(1)
            ...
            pass
    _WORKER_DOMAIN = prepare_domain(cfg)


def _run_phase3_sample_task(task: Tuple[float, int, np.ndarray, str, str]) -> Dict[str, Any]:
    global _WORKER_CFG, _WORKER_DOMAIN
    cfg = _WORKER_CFG
    domain = _WORKER_DOMAIN
    if cfg is None or domain is None:
        raise RuntimeError("Phase 3 worker not initialized")
    mw, sample_index, slip, slip_name, mag_dir_s = task
    mag_dir = Path(mag_dir_s)
    start = time.perf_counter()
    sample = run_tsunami_simulation(cfg, domain, slip, slip_name, sample_index, forcing_mode="dynamic", pbar=None)
    if domain.nested_local_domain is not None and sample.nested_time_sec is not None and sample.nested_eta_box is not None and sample.nested_time_sec.size >= 2:
        print(f"[STATUS] Sample {sample_index}: regional corridor run finished. Building local boundary driver ...", flush=True)
        driver = build_local_boundary_driver(domain, domain.nested_local_domain, sample.nested_time_sec, sample.nested_eta_box)
        driver_diag = compute_driver_diagnostics(driver, domain.nested_local_domain)
        driver, window_meta = trim_driver_to_active_window(driver, cfg)
        sample.meta["local_active_window"] = window_meta
        sample.meta["local_driver_diagnostics"] = {**driver_diag, **window_meta}
        print(f"[STATUS] Sample {sample_index}: turning over to local Dapa nested grid ({domain.nested_local_domain.grid.shape[0]}x{domain.nested_local_domain.grid.shape[1]}) for t={window_meta['t_start_sec']:.1f}..{window_meta['t_end_sec']:.1f}s | peak boundary eta={driver_diag['peak_boundary_eta_m']:.3f} m | wet edges B/T/L/R={driver_diag['bottom']}/{driver_diag['top']}/{driver_diag['left']}/{driver_diag['right']} ...", flush=True)
        sample.local_nested = run_local_nested_simulation(cfg, domain.nested_local_domain, driver, pbar=None)
        sample.local_nested.meta["active_window"] = window_meta
        sample.local_nested.meta["driver_diagnostics"] = driver_diag
        print(f"[STATUS] Sample {sample_index}: local Dapa nested grid finished. local max depth={float(np.max(sample.local_nested.max_depth)):.3f} m | nonzero inundation cells={int(np.count_nonzero((sample.local_nested.max_depth > 0.0) & (domain.nested_local_domain.grid.z > 0.0)))} | local Dapa gauge max={float(sample.local_nested.gauge_max_abs.get('Dapa_virtual', 0.0)):.3f} m", flush=True)
    runtime_sec = time.perf_counter() - start
    outputs = persist_phase3_sample_outputs(cfg, domain, sample, mw, sample_index, slip_name, mag_dir)
    local_gauge_vals = sample.local_nested.gauge_max_abs if sample.local_nested is not None else {}
    gauge_row = {"sample_index": sample_index, "slip_id": slip_name, "forcing_mode": sample.forcing_mode, **sample.gauge_max_abs}
    for k, v in local_gauge_vals.items():
        gauge_row[k] = float(v)
    catalog_row = scenario_summary_row(mw, sample, runtime_sec, domain, use_local=True)
    if sample.local_nested is not None:
        catalog_row["local_nonzero_inundation_cells"] = int(np.count_nonzero((sample.local_nested.max_depth > 0.0) & (domain.nested_local_domain.grid.z > 0.0)))
    else:
        catalog_row["local_nonzero_inundation_cells"] = 0
    if sample.meta.get("local_driver_diagnostics"):
        catalog_row.update(sample.meta["local_driver_diagnostics"])
    return {
        "gauge_row": gauge_row,
        "p95_row": {"sample_index": sample_index, "slip_id": slip_name, "dapa_p95_inundation_m": sample.local_nested.dapa_p95_inun if sample.local_nested is not None else sample.dapa_p95_inun},
        "catalog_row": catalog_row,
        "regional_depth_npy": outputs.get("regional_depth_npy"),
        "local_depth_npy": outputs.get("local_depth_npy"),
    }
...
def run_dapa_ensemble(cfg: ProjectConfig, domain: PreparedDomain, mw: float, n_samples: int) -> Dict[str, Any]:
    ...
    regional_depth_paths: List[str] = []
    local_depth_paths: List[str] = []

    if cfg.execution.workers > 1:
        maybe_print(f"[STATUS] Parallel sample workers enabled: {cfg.execution.workers}", pbar)
        tasks = [(mw, i, slip, slip_name, str(mag_dir)) for i, (slip, slip_name) in enumerate(samples)]
        with ProcessPoolExecutor(max_workers=cfg.execution.workers, initializer=_init_phase3_worker, initargs=(cfg,)) as ex:
            futs = [ex.submit(_run_phase3_sample_task, task) for task in tasks]
            done = 0
            for fut in as_completed(futs):
                res = fut.result()
                gauge_rows.append(res["gauge_row"])
                p95_rows.append(res["p95_row"])
                catalog_rows.append(res["catalog_row"])
                if res.get("regional_depth_npy"):
                    regional_depth_paths.append(res["regional_depth_npy"])
                if res.get("local_depth_npy"):
                    local_depth_paths.append(res["local_depth_npy"])
                done += 1
                pbar.update(1)
                if done % cfg.execution.checkpoint_every == 0 or done == len(samples):
                    maybe_print(f"[CHECKPOINT] Mw{mw:.1f}: completed {done}/{len(samples)} samples", pbar)
                    save_checkpoint(mag_dir, mw, done - 1, gauge_rows, p95_rows)
    else:
        for i, (slip, slip_name) in enumerate(samples):
            ...
            if domain.nested_local_domain is not None and sample.nested_time_sec is not None and sample.nested_eta_box is not None and sample.nested_time_sec.size >= 2:
                maybe_print(f"[STATUS] Sample {i}: regional corridor run finished. Building local boundary driver ...", pbar)
                driver = build_local_boundary_driver(domain, domain.nested_local_domain, sample.nested_time_sec, sample.nested_eta_box)
                driver_diag = compute_driver_diagnostics(driver, domain.nested_local_domain)
                driver, window_meta = trim_driver_to_active_window(driver, cfg)
                sample.meta["local_active_window"] = window_meta
                sample.meta["local_driver_diagnostics"] = {**driver_diag, **window_meta}
                maybe_print(f"[STATUS] Sample {i}: turning over to local Dapa nested grid ({domain.nested_local_domain.grid.shape[0]}x{domain.nested_local_domain.grid.shape[1]}) for t={window_meta['t_start_sec']:.1f}..{window_meta['t_end_sec']:.1f}s | peak boundary eta={driver_diag['peak_boundary_eta_m']:.3f} m | wet edges B/T/L/R={driver_diag['bottom']}/{driver_diag['top']}/{driver_diag['left']}/{driver_diag['right']} ...", pbar)
                sample.local_nested = run_local_nested_simulation(cfg, domain.nested_local_domain, driver, pbar)
                sample.local_nested.meta["active_window"] = window_meta
                sample.local_nested.meta["driver_diagnostics"] = driver_diag
                maybe_print(f"[STATUS] Sample {i}: local Dapa nested grid finished. local max depth={float(np.max(sample.local_nested.max_depth)):.3f} m | nonzero inundation cells={int(np.count_nonzero((sample.local_nested.max_depth > 0.0) & (domain.nested_local_domain.grid.z > 0.0)))} | local Dapa gauge max={float(sample.local_nested.gauge_max_abs.get('Dapa_virtual', 0.0)):.3f} m", pbar)
            runtime_sec = time.perf_counter() - start
            gauge_row = {"sample_index": i, "slip_id": slip_name, "forcing_mode": sample.forcing_mode, **sample.gauge_max_abs}
            if sample.local_nested is not None and sample.local_nested.gauge_max_abs:
                for k, v in sample.local_nested.gauge_max_abs.items():
                    gauge_row[k] = float(v)
            gauge_rows.append(gauge_row)
            p95_rows.append({"sample_index": i, "slip_id": slip_name, "dapa_p95_inundation_m": sample.local_nested.dapa_p95_inun if sample.local_nested is not None else sample.dapa_p95_inun})
            catalog_rows.append(scenario_summary_row(mw, sample, runtime_sec, domain, use_local=True))
            outputs = persist_phase3_sample_outputs(cfg, domain, sample, mw, i, slip_name, mag_dir)
            if outputs.get("regional_depth_npy"):
                regional_depth_paths.append(outputs["regional_depth_npy"])
            if outputs.get("local_depth_npy"):
                local_depth_paths.append(outputs["local_depth_npy"])
            pbar.update(1)
    ...
    gauge_df = pd.DataFrame(gauge_rows).sort_values("sample_index").reset_index(drop=True)
    p95_df = pd.DataFrame(p95_rows).sort_values("sample_index").reset_index(drop=True)
    catalog_df = pd.DataFrame(catalog_rows).sort_values("sample_index").reset_index(drop=True)
    ...
    stack = np.stack([np.load(p).astype(np.float32) for p in sorted(regional_depth_paths)], axis=0)
    ...
    export_map_bundle(mag_dir, f"Mw{mw:.1f}_regional_p95", domain.grid, regional_p95, np.zeros_like(regional_p95), np.zeros_like(regional_p95), np.full_like(regional_p95, np.nan), (regional_p95 > 0).astype(np.uint8), write_geotiff=cfg.execution.write_geotiff, write_npz=cfg.execution.write_npz)
    if cfg.execution.write_png:
    ...
    export_map_bundle(mag_dir, f"Mw{mw:.1f}_dapa_corridor_p95", dapa_grid, dapa_corridor_p95, np.zeros_like(dapa_corridor_p95), np.zeros_like(dapa_corridor_p95), np.full_like(dapa_corridor_p95, np.nan), (dapa_corridor_p95 > 0).astype(np.uint8), write_geotiff=cfg.execution.write_geotiff, write_npz=cfg.execution.write_npz)
    if cfg.execution.write_png:
    ...
    local_p95_map = None
    if local_depth_paths and domain.nested_local_domain is not None:
        local_stack = np.stack([np.load(p).astype(np.float32) for p in sorted(local_depth_paths)], axis=0)
        ...
        export_map_bundle(mag_dir, f"Mw{mw:.1f}_dapa_local_p95", domain.nested_local_domain.grid, local_p95_map, np.zeros_like(local_p95_map), np.zeros_like(local_p95_map), np.full_like(local_p95_map, np.nan), (local_p95_map > 0).astype(np.uint8), write_geotiff=cfg.execution.write_geotiff, write_npz=cfg.execution.write_npz)
        if cfg.execution.write_png:
    ...
    if cfg.execution.emit_phase4_preview:
        for thr in cfg.probability.thresholds_m:
            ...
            export_map_bundle(mag_dir, f"Mw{mw:.1f}_conditional_exceedance_{thr:.1f}m", domain.grid, ape, np.zeros_like(ape), np.zeros_like(ape), np.full_like(ape, np.nan), (ape > 0).astype(np.uint8), write_geotiff=cfg.execution.write_geotiff, write_npz=cfg.execution.write_npz)
            if cfg.execution.write_png:
                save_map(domain.grid, ape, f"Mw{mw:.1f} | P(depth>{thr:.1f}m)", mag_dir / f"Mw{mw:.1f}_conditional_exceedance_{thr:.1f}m.png")

    viability = build_viability_report(cfg, domain, regional_p95, p95_df, local_p95_map=local_p95_map, local_domain=domain.nested_local_domain, catalog_df=catalog_df)
    save_json(mag_dir / f"Mw{mw:.1f}_viability_report.json", viability)
    save_json(mag_dir / f"Mw{mw:.1f}_summary.json", {
        "mw": mw,
        "n_samples": n_samples,
        "mean_max_wave_by_gauge_m": viability.get("mean_max_wave_by_gauge_m", {g.name: float(gauge_df[g.name].mean()) for g in cfg.gauges if g.name in gauge_df}),
...
def build_config(args: argparse.Namespace) -> ProjectConfig:
    ...
    cfg.execution.nested_diag_stride = args.nested_diag_stride
    cfg.execution.workers = args.workers
    cfg.execution.local_active_threshold_m = args.local_active_threshold_m
    cfg.execution.local_active_prebuffer_sec = args.local_active_prebuffer_sec
    cfg.execution.local_active_postbuffer_sec = args.local_active_postbuffer_sec
    ...
    if args.phase3_production_lite:
        apply_phase3_production_lite_preset(cfg)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--nested-diag-stride", type=int, default=500, help="Status-report interval for the local nested Dapa run")
    ...
    p.add_argument("--phase3-production-lite", action="store_true", help="Compact Phase 3 production-lite mode: local nested numerical bundles per sample, no heavy PNG/GeoTIFF cartography, 7200 s default, and worker-friendly bulk output.")
    p.add_argument("--workers", type=int, default=1, help="Number of parallel sample workers for dapa-ensemble mode.")
    p.add_argument("--local-active-threshold-m", type=float, default=0.02, help="Boundary eta threshold used to detect the active local nested window.")
    p.add_argument("--local-active-prebuffer-sec", type=float, default=600.0, help="Seconds of buffer before first meaningful boundary arrival for the local nested window.")
    p.add_argument("--local-active-postbuffer-sec", type=float, default=900.0, help="Seconds of buffer after the last meaningful boundary arrival for the local nested window.")
...
def main() -> None:
    ...
    if cfg.execution.phase3_production_lite:
        print("[NOTE] Phase 3 production-lite mode is ON: compact per-sample local nested numerical bundles will be written for downstream aggregation.", flush=True)
        print(f"[NOTE] Workers={cfg.execution.workers} | t_end_sec={cfg.solver.t_end_sec:.0f} | local active window threshold={cfg.execution.local_active_threshold_m:.3f} m", flush=True)
